...
export default function Register() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const { data } = await api.post<{ token: string; name: string }>('/auth/register', {
        name: form.name, email: form.email, password: form.password,
      });
      login(data.token, data.name, form.email);
        ...
        <div className="bg-slate-900/80 backdrop-blur-xl border border-slate-800 rounded-3xl p-8 shadow-2xl">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="relative">
              ...
              <input className={inputCls} type="email" placeholder="Email address" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} required />
            </div>
            ...
            <div className="relative">
              <Lock size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500" />
              <input
                className={`${inputCls} pr-10 ${passwordMatch ? 'border-green-500/50' : passwordMismatch ? 'border-red-500/50' : ''}`}
            ...
            {passwordMismatch && <p className="text-red-400 text-xs -mt-2">Passwords do not match</p>}
            {passwordMatch    && <p className="text-green-400 text-xs -mt-2">✓ Passwords match</p>}
          ...
          <p className="text-center text-slate-400 text-sm mt-6">
            ...
            <Link href="/login" className="text-brand hover:text-brand-light font-medium transition-colors">Sign in</Link>
          </p>
        </div>
      </motion.div>
    </div>
